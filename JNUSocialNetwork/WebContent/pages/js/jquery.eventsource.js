...
(function(jQuery, global) {
	...
	pluginFns = {
		...
		_private : {
			...
			openEventSource : function(options) {
				...
				stream.cache[label].stream.addEventListener("open", function(
						event) {
						...
						.addEventListener(
								...
								function(event) {
									...
									if (stream.cache[label]) {
										...
										streamData[streamData.length] = jQuery
										...
										stream.cache[label].options.message
												.call(
														this,
														streamData[0] ? streamData[0]
																: null,
														{
															data : streamData,
			...
			openPollingSource : function(options) {
				var label = options.label, source;
					...
					source = jQuery
							.ajax({
								...
								beforeSend : function() {
									if (stream.cache[label]) {
										this.label = label;
										stream.cache[label].options.open
												.call(this);
									}
								},
								success : function(data) {

									var tempdata, label = options.label, parsedData = [], streamData = jQuery
											.map(data.split("\n\n"), function(
													sdata, i) {
												return !!sdata && sdata;
											}), idx = 0, length = streamData.length, rretryprefix = /retry/, retries;
									...
									if (stream.cache[label]) {
										...
										setTimeout(
												function() {
													pluginFns._private.openPollingSource
															.call(this, options);
												},
												// Use server sent retry time if
												// exists or default retry time
												// if not
	...
	jQuery.eventsource = function(options) {
		...
		// Plugin sub function
		if (options && !jQuery.isPlainObject(options)
		...
		options.data = options.data && jQuery.isPlainObject(options.data) ? jQuery
				.param(options.data)
				: options.data;
		...
		options.label = !options.label ? options.url + "?" + options.data
				: options.label;
		...
		pluginFns._private.openPollingSource(opts) : new EventSource(opts.url
				+ (opts.data ? "?" + opts.data : ""));
