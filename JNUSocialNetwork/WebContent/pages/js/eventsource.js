...
(function(global) {
		...
		ondata : function() {
			...
			if (request.isReady() && !request.hasError()) {
				...
				if (this.cursor == 0 && buffer.length > 0) {
					...
					// skip byte order mark \uFEFF character if it starts the
					// stream
		...
		parseStream : function(chunk) {
			...
			for (i = 0; i < (events.length - 1); i++) {
				...
				for (j = 0; j < parts.length; j++) {
					...
					} else if (line.indexOf('id') == 0) { // this resets the
						// id
	...
	function isOldIE() {
		...
		return (window.XDomainRequest && (window.XMLHttpRequest && new XMLHttpRequest().responseType === undefined)) ? true
				: false;
	}
