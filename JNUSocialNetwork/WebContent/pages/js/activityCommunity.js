                  ...
                  function() {
                    if ($('#activityTime').val() != ""
                      ...
                      if ($('#fileuploadA').val() != "") {
                        ...
                        post.attributes["background"] = FileUpload(new FormData($('.activityForm')[0]))[0];
                      }
                      if ($('#fileuploadB').val() != "") {
                        post.attributes["registerTemplateAddr"] = RegisterFormUpload(new FormData($('.regForm')[0]));
                      }
  ...
  $.each(response.reverse(), function(n, dataString) {
    if (dataString.available == true) {
      addActivity(dataString.ID, dataString.attributes.activityName,
              ...
              dataString.participantIDs, dataString.attributes.startDate,
              dataString.attributes.limitation, dataString.attributes.ifUpload,
...
$('body').on(
        ...
        function() {
          if ($('.uploadForm')[0].checkValidity()
            ...
            var response = formUpload(new FormData($('.uploadForm')[0]), $(
                    '.ulR').attr('id'), encodeURI($.parseJSON(sessionStorage
...
function toTimeValue(dateTime) {
  ...
  var d = new Date(matchers[3], month, matchers[2], matchers[4], matchers[5], 0, 0);
  return d.valueOf();
                          ...
                          .click(function(event) {
                            if (!$('.activityForm')[0].checkValidity()
                              ...
                              return;
                                    ...
                                    && !/.*\.xls|.xlsx|.doc|.docx|.wps|.et$/
                                            .test($('#fileuploadB').val())) {
                              ...
                              if (current_page == 2) {
                                ...
                                $('#table_activityRemind').html(
                                        $('#activityRemind').val());
                                $('#table_activityAddr').html(
                                        $('#activityAddr').val().length > 30
                                                ? $('#activityAddr').val()
                                                        .substring(0, 30)
                                                        + '......' : $(
                                                        '#activityAddr').val());
                                $('#table_activityMore').html(
                                        $('#activityMore').val().length > 30
                                                ? $('#activityMore').val()
                                                        .substring(0, 30)
                                                        + '......' : $(
                                                        '#activityMore').val());
                                ...
                                $('#table_activitySign').html(
                                        $('#optionsRadios1').is(':checked')
                                                ? '默认方式' : '上传报名表');
                                $('#table_fileuploadA').html(
                                        $('#fileuploadA').val() == '' ? '否'
                                                : $('#fileuploadA').val());
                                $('#table_fileuploadB').html(
                                        $('#fileuploadB').val() == '' ? '否'
                                                : $('#fileuploadB').val());
                              }
                            }
                  ...
                  if ($(window).scrollTop() == $(document).height()
                    ...
                    if (response.length != 0) {
                      $.each(response, function(n, dataString) {
                        if (dataString.available == true
                                && $("div[class='activity post" + dataString.ID
                                        + "']").length == 0) {
