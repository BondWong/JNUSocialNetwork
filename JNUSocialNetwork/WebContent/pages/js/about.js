$(document) .ready( function () 
{
    ...
    $('#fileuploadPhoto') .fileupload( 
    {
        ...
        acceptFileTypes : /(\.|\/)(gif|jpe?g|png)$/i, maxFileSize : 5000000 // 5 MB
...
function aboutClickEvent() 
{
    ...
    // 标签组提示
    var tagI = [ "单身待解救", "奋斗ing", "幸福ing", "成长ing", "缺爱ing", "静待缘分", "心如止水", "寂寞ing", "求职ing", "考研ing", 
    "备战雅思", "备战托福", "我是小鲜肉" ];
    $('body').on( "click", ".changeTag0", function () 
    {
        $('.tagName').text("你现在的状态？");
        $('.tagContainer').remove();
        var tagArray = "";
    ...
    var tagD = [ "幽默", "乐观", "低调", "完美主义", "三分钟热度", "善良", "阳光", "直率", "执着", "体贴", "内敛", "温柔", "自信", "呆萌", 
    "纠结" ];
    ...
    var tagE = [ "周杰伦", "林俊杰", "王力宏", "五月天", "孙燕姿", "邓紫棋", "李敏浩", "金秀贤", "汤姆汉克斯", "梁朝伟", "吴彦祖", "莱昂纳多", 
    "C罗" ];
    ...
    $('body') .on( "click", ".aEditbtn2", function () 
    {
        var userInfo = FetchUserByID(userID);
        $("span[class='Alooking']").html( "<textarea class='lookingforE' style='resize: none;' maxLength='30'>" + userInfo.attributes.introduce + "</textarea>");
        $("span[class='Arelationship']") .html( "<select class='relationshipnE' val='as'><option value=''>请选择</option><option value='没有目标'>没有目标</option><option value='单身'>单身</option><option value='有暗恋对象'>有暗恋对象</option><option value='恋爱中'>恋爱中</option><option value='准备分手'>准备分手</option><option value='已经分手'>已经分手</option></select>");
        $( "option[value='" + userInfo.attributes.relationship + "']").attr("selected", "selected");
        $(this).text("保存");
        $(this).attr("class", "btn btn-primary aSavebtn2");
    });
    $('body') .on( "click", ".aEditbtn3", function () 
    {
        var userInfo = FetchUserByID(userID);
        $("span[class='Cintro']") .html( "<textarea class='cintroE' style='resize: none;' id='focusedInput' type='text' value='' maxLength='30'>" + userInfo.attributes.introduce + "</textarea>");
    ...
    $('body').on( 'click', '.aSavebtn', function () 
    {
        ...
        var datajson = 
        {
            telnum : $('.telenumE').val(), email : $('.emailE').val(), dorm : $('.addressE').val(), wechat : $('.wechatE').val(), 
            selectTele : $('.selectTele').val(), selectEmail : $('.selectEmail').val(), selectWechat : $('.selectWechat').val(), 
            selectAddre : $('.selectAddre').val(), selectBirth : $('.selectBirth').val() 
        };
    ...
    $('body').on('click', '.aSavebtn2', function () 
    {
        $('.selectAbout').css("display", "none");
        var datajson = {
            introduce : $('.lookingforE').val(), relationship : $('.relationshipnE').val(), 
        };
        if ($('.nameE').val() != null) 
        {
            datajson = {
    ...
    $('body').on( 'click', '.aSavebtn3', function () 
    {
        ...
        var datajson = 
        {
            name : $('.cnicknameE').val(), introduce : $('.cintroE').val(), Cinstitution : $('.cinstitutionE').val(), 
            Ccontact : $('.ccontactE').val(), 
        };
        var json = $.toJSON(datajson);
        var userInfo = UpdateUserProfile(USERID, json);
        communityShow(userInfo.attributes.name, userInfo.attributes.introduce, userInfo.attributes.Cinstitution, 
    ...
    (function () 
    {
        $(function () 
        {
            ...
            {
                aspectRatio : 2.067, imgPreferredSize : 5, imgUrlAttrName : 'profileImageLink', targetView : ".profile_img" 
            });
        });
    })(jQuery);
    ...
    $ .each( response.reverse(), function (n, dataString) 
    {
        if (dataString.postType == "NORMAL" && dataString.available == true) 
        {
            ...
            if (dataString.attributes.communityName != null) 
            {
                ...
                $(".postComm" + dataString.ID).children( '.deletePostBtn').attr('class', 'deletePostFromCBtn').attr('id', 
...
{
    open_chatroom_with_message(USERID, userID, $('.profile_user_name') .text(), 'hi');
});
//发邮件功能
