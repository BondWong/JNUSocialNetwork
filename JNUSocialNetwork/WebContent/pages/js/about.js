...
function aboutClickEvent() {
	...
	/*
	 * // function avatarImgBtn $('body') .on( "click", ".avatarImgBtn",
	 * function() { var datajson = { avatarLink : FileUpload(new FormData(
	 * $('.avatarForm')[0]))[0], }; var json = $.toJSON(datajson); var userNew =
	 * UpdateUserProfile(userID, json);
	 * $('.profile_user_img').find('img').attr("src",
	 * $.parseJSON(userNew.attributes.avatarLink).src);
	 * $('#myModal').modal('hide'); $('.avatarForm').get(0).reset();
	 * 
	 * }); // change Background $('body').on( "click", ".changeBg", function() {
	 * var datajson = { profileImageLink : FileUpload(new FormData(
	 * $('.changBgForm')[0]))[0], }; var json = $.toJSON(datajson); var userNew =
	 * UpdateUserProfile(userID, json); ; $('#myModalB').modal('hide');
