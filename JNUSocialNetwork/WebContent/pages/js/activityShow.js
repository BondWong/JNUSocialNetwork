...
function showActivityDetail(activity, community) {
  if (activity.attributes.ifUpload == "默认方式") {
    $('#download-name-list-button').attr(
      "href",
      "../app/fileDownloader?type=ACTIVITYNAMELIST&version=2007+&activityID=" + activity.ID);
  } else {
    $('#download-name-list-button').attr(
      "href",
      "../app/fileDownloader?type=ACTIVITYREGISTERS&activityID=" + activity.ID);
    /*$('.joinSActivity')
				.replaceWith(
  ...
  $
    .each(
      ...
      function (index, jsonComment) {
        ...
        var commentReply = "<div class='comment_reply replyaComment' id=" + activity.ID + " style='cursor: pointer'><a><input id='replyName' type='hidden' value='" + jsonComment.owner.attributes.name + "' /><input id='replyID' type='hidden' value='" + jsonComment.ID + "' />reply<span style='font-size: 8px'></span></a></div>";
        var removeBtn = "";
        if (USERID == jsonComment.owner.ID) {
          removeBtn = "<div class='deleteCommBtn deletCa' style='cursor:pointer'><a><input id='" + jsonComment.attributes.postID + "' type='hidden' value='" + jsonComment.ID + "' /><span class='glyphicon glyphicon-remove' style='font-size: 8px'></span></a></div>";
          commentReply = "";
        }
...
function showImages() {
  if (activity.imageLinks.length == 0) {
    ...
    $.each(activity.imageLinks, function (index, imageLink) {
      if (index == 0) {
        var photoContainer = "<img width='600' height='450' src='" + $.parseJSON(decodeURIComponent(imageLink)).src + "' />";
...
$('#inviteFriendsButton').click(function(){
	$('#invite_friends_modal_contener').css({'background':'url("images/loading.gif") no-repeat center'});
	if(friendList.length==0){
		$.ajax({
			url:'/app/user/fetchFolloweesWithEmail/'+USERID+'/'+0+'/'+$.parseJSON(sessionStorage.getItem("user")).followeeWithEmailIDs.length,
			dataType:'json',
			type:'GET',
			success:function(data){
				for(var i=0;i<data.length;i++){
		...
		$('#invite_friends_modal_contener').css({'background':'none'});
	}
});
function init(){
	...
	if(friendList.length==0){
		...
		for(var i=0;i<page1size;i++){
			var friendbox=$("<div data="+i+" class='friendbox' active='false'><div data="+i+" class='head'><img data="+i+" src='"+friendList[i].attributes.avatarLink.substr(8,friendList[i].attributes.avatarLink.indexOf(',')-9)+"'/></div><div data="+i+" class='name'>"+friendList[i].attributes.name+"</div></div>");
			if(selectedIndexs[i]){
				friendbox.css({backgroundColor:'#3071a9',color:'#ffffff'});
				friendbox.attr('active','true');
			}
			friendbox.appendTo($('#invite_friends_modal_contener'));
		}
		
		$('.friendbox').click(function(event){
			...
			if(target.attr('active')=='false'){
				if(selectedFriendNumber==maxInviteNumber){
					$('#max_invite_friends_alert').fadeIn(easingtime).delay(2000).fadeOut(easingtime);
					return;
				}
				target.animate({backgroundColor:'#3071a9',color:'#ffffff'},easingtime);
		...
		var pageNum=Math.ceil($.parseJSON(sessionStorage.getItem("user")).followeeWithEmailIDs.length/6);
		var pageCurrent=1;
		...
		function pageRun(index){
			...
			var pagensize=friendList.length-(index-1)*6>6?6:friendList.length-(index-1)*6;
			$('#invite_friends_modal_contener').animate({opacity:'0'},easingtime,function(){
				$('#invite_friends_modal_contener').empty();
				for(var i=(index-1)*6;i<(index-1)*6+pagensize;i++){
					var friendbox=$("<div data="+i+" class='friendbox' active='false'><div data="+i+" class='head'><img data="+i+" src='"+friendList[i].attributes.avatarLink.substr(8,friendList[i].attributes.avatarLink.indexOf(',')-9)+"'/></div><div data="+i+" class='name'>"+friendList[i].attributes.name+"</div></div>");
					if(selectedIndexs[i]){
						friendbox.css({backgroundColor:'#3071a9',color:'#ffffff'});
						friendbox.attr('active','true');
			...
			$('#invite_friends_modal_contener').animate({opacity:'1'},easingtime);
			pageCurrent=index;
		};
	}	
	$('#sendInvite').click(function(){
		...
		$.ajax({
			url:'../../app/user/canSendInvitation/'+USERID+'/'+activityID,
			dataType:'json',
			type:'GET',
			error:function(XMLHttpRequest, textStatus, errorThrown) {
				if(XMLHttpRequest.status!=200){
					$('#cannot_invite_friends_alert').fadeIn(easingtime).delay(2000).fadeOut(easingtime);
				}else{
					var emails=new Array();
					for(var i=0;i<friendList.length;i++){
						if(selectedIndexs[i]){
							emails.push(friendList[i].attributes.email);
						}
					}
					...
					$.ajax({
						type:'POST',
						url:'../../app/user/sendInvitation/'+USERID+'/'+activityID,
						dataType:'json',
