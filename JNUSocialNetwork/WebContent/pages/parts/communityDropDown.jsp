<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>

<ul class="nav navbar-nav">
	<li class="dropdown"><a href="#" class="dropdown-toggle"
		data-toggle="dropdown"><span class="glyphicon glyphicon-th-large">Community</span><b
			class="caret"></b></a>
		<ul class="dropdown-menu">
			<li><a href="home.jsp"><span
					class="glyphicon glyphicon-home">&nbsp;Home</span></a></li>
			<li><a href="community.jsp"><span
					class="glyphicon glyphicon-th">&nbsp;Communities</span></a></li>
			<li><a href="circle.jsp"><span
					class="glyphicon glyphicon-comment">&nbsp;Chat</span></a></li>
			<li><a href="people.jsp"><span
					class="glyphicon glyphicon-magnet">&nbsp;people</span></a></li>
		</ul></li>
</ul>
<c:choose>
	<c:when test="${param.nav eq 'home' }">
		<ul class="nav navbar-nav">
			<li class="dropdown"><a href="#" class="dropdown-toggle"
				data-toggle="dropdown"><span class="glyphicon glyphicon-home">&nbsp;Home</span><b
					class="caret"></b></a>
				<ul class="dropdown-menu">
					<li><a href="community.jsp"><span
							class="glyphicon glyphicon-th">&nbsp;Communities</span></a></li>
					<li><a href="circle.jsp"><span
							class="glyphicon glyphicon-comment">&nbsp;Chat</span></a></li>
					<li><a href="people.jsp"><span
							class="glyphicon glyphicon-magnet">&nbsp;people</span></a></li>
				</ul></li>
		</ul>
	</c:when>
	<c:when test="${param.nav eq 'community' }">
		<ul class="nav navbar-nav">
			<li class="dropdown"><a href="#" class="dropdown-toggle"
				data-toggle="dropdown"><span class="glyphicon glyphicon-th">&nbsp;Community</span><b
					class="caret"></b></a>
				<ul class="dropdown-menu">
					<li><a href="home.jsp"><span
							class="glyphicon glyphicon-home">&nbsp;Home</span></a></li>
					<li><a href="circle.jsp"><span
							class="glyphicon glyphicon-comment">&nbsp;Chat</span></a></li>
					<li><a href="people.jsp"><span
							class="glyphicon glyphicon-magnet">&nbsp;people</span></a></li>
				</ul></li>
		</ul>
	</c:when>
	<c:when test="${param.nav eq 'circle' }">
		<ul class="nav navbar-nav">
			<li class="dropdown"><a href="#" class="dropdown-toggle"
				data-toggle="dropdown"><span class="glyphicon glyphicon-comment">&nbsp;Chat</span><b
					class="caret"></b></a>
				<ul class="dropdown-menu">
					<li><a href="home.jsp"><span
							class="glyphicon glyphicon-home">&nbsp;Home</span></a></li>
					<li><a href="community.jsp"><span
							class="glyphicon glyphicon-th">&nbsp;Communities</span></a></li>
					<li><a href="people.jsp"><span
							class="glyphicon glyphicon-magnet">&nbsp;people</span></a></li>
				</ul></li>
		</ul>
	</c:when>
	<c:when test="${param.nav eq 'people' }">
		<ul class="nav navbar-nav">
			<li class="dropdown"><a href="#" class="dropdown-toggle"
				data-toggle="dropdown"><span
					class="glyphicon glyphicon-magnet">&nbsp;people</span><b
					class="caret"></b></a>
				<ul class="dropdown-menu">
					<li><a href="home.jsp"><span
							class="glyphicon glyphicon-home">&nbsp;Home</span></a></li>
					<li><a href="community.jsp"><span
							class="glyphicon glyphicon-th">&nbsp;Communities</span></a></li>
					<li><a href="circle.jsp"><span
							class="glyphicon glyphicon-comment">&nbsp;Chat</span></a></li>
				</ul></li>
		</ul>
	</c:when>
</c:choose>